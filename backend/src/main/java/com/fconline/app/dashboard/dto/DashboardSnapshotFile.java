...
/**
 * data/dashboard-snapshot.json(CUSTOM) · data/dashboard-snapshot-official.json(OFFICIAL) 파일
 * 스키마 — 스코프 하나당 파일 하나(요청, 2026-09-04 확대: 원래는 CUSTOM 전용이었다). site-root/
 * report.html(유저 칩 "전체")이 백엔드를 전혀 거치지 않고 raw.githubusercontent.com에서 현재
 * 매치타입 토글에 맞는 파일을 직접 fetch한다(백엔드 콜드 스타트와 무관하게 즉시 뜨게 하려는
 * 목적 — data/insight-snapshots/*.json과 같은 컨벤션).
 *
 * matchType은 이 스냅샷이 어느 스코프인지 프론트가 URL 대신 파일 내용만으로도 알 수 있게 담아둔다
 * (표시 텍스트에 "커스텀"/"공식전" 라벨을 붙이는 용도).
 *
